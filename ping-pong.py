...
def calibrate_timer_and_events():
    ...
    global chanse
    global timer_event
    global timer_event_i
    global end
    randomised_num = randint(1,chanse)
    if second_timer > 9:
        tsecond_timer += 1
        second_timer = 0
        chanse = chanse-chanse//20
    if tsecond_timer == 6:
        minut_timer += 1
        tsecond_timer = 0 
        ball.xspeed += 3
    ...
    if timer_event_i > 9:
        ...
        timer_event_i = 0
        timer_event = False
player1 = Player(10,'player1.png',650,220)
player2 = Player(10,'player2.png',100,220)
ball = Ball(3,'ball.png',400,100)
ball2 = Ball(3,'ball.png',400,100)
second_timer = 0
minut_timer = 0
tsecond_timer = 0
tminut_timer = 0
...
while game:
    ...
    if end:
        ...
        player2.move2()
        ball.update()
        if ball.rect.x <= -110 or ball2.rect.x <= -110:
            rounds += 1
            blue_goals += 1
            blue = font.render(str(blue_goals),True,(0,50,255))
            ball.rect.x = 400
            ...
            player2.rect.y = 220
        window.blit(blue,(650,50))    
        if ball.rect.x >= 910 or ball2.rect.x >= 910:
            rounds += 1
            red_goals += 1
            red = font.render(str(red_goals),True,(255,50,50))
            ball.rect.x = 400
